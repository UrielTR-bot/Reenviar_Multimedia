...
# ----------------------------
pending_mirrors = {}  # channel_msg_id -> etiqueta
...
ALBUM_TIMEOUT = 10  # Espera para "cerrar" un álbum
...
def asegurar_etiqueta_y_espelho(etiqueta: str) -> int:
    """
    1. Consulta la DB: si la etiqueta ya existe, retorna su group_msg_id.
    2. Si no existe, crea un post en el canal y registra pending_mirrors[post_id] = etiqueta.
    3. Espera hasta 10s a que se capture el espejo.
    """
...
def finalize_album(media_group_id):
    """
    Llamado cuando se cierra el álbum (ALBUM_TIMEOUT).
    Enviamos un "media group" distinto para cada etiqueta,
    reescribiendo la caption para que solo contenga esa etiqueta.
    """
    ...
    # Para cada etiqueta, enviamos el álbum con "send_media_group"
    for tag in all_tags:
        ...
        first = True
        for am in msgs:
            # Reescribimos la caption para que solo contenga 'tag'
            original_caption = am.caption or ""
            if first and original_caption:
                # Quitar las otras etiquetas
...
def on_private_message(client: Client, message: Message):
    """
    - Si es parte de un álbum, lo bufferizamos y esperamos ALBUM_TIMEOUT.
    - Si es un mensaje suelto, lo enviamos con send_* en cada etiqueta, reescribiendo la caption.
    """
    if message.media_group_id:
        ...
        # Para cada etiqueta, enviamos un "nuevo" mensaje usando send_photo/video/etc.
        for tag in found_tags:
            ...
            if message.photo:
                ...
                # Si es texto u otro tipo no manejado
                bot.send_message(
                    chat_id=DISCUSSION_ID,
                    text=nueva_caption,
                    reply_to_message_id=group_msg_id
